#' oasisuiIncrementalPanel
#'
#' @rdname oasisuiIncrementalPanel
#'
#' @description Incremental oasisui panel module, including a button to add
#'   a new panel above.
#'
...
#'
#' @md
oasisuiIncrementalPanel <- function(input, output, session, panels_state,
  ...
  observeEvent(input$add, {
    ...
    if (length(new_id) > 0) {
      ...
      insertUI(
        ...
        oasisuiIncrementalPanelUI(new_id, new_content_fun(new_content_id, ...), heading = new_headings[[new_id]],
                                   collapsible = collapsible, show = show)
      )
    } else {
      oasisuiNotification(type = "warning",
...
#' @rdname oasisuiIncrementalPanel
#'
#' @inheritParams oasisuiPanel
#' @param removable Logical flag specifying if the panel can be removed.
#'
#' @export
#'
#' @md
oasisuiIncrementalPanelUI <- function(id, ..., heading = NULL, footer = NULL, status = "default",
...
#' [oasisuiIncrementalPanel]s, returning their reactive state and a function to
#' remove them all.
#'
...
#' @inheritParams oasisuiIncrementalPanel
#'
#' @return
...
callIncrementalPanelModules <- function(IDs, ID_0,
  ...
  lapply(
    c(IDs, ID_0),
    callModule, module = oasisuiIncrementalPanel,
    panels_state,
    contentIDs, contentUI,
