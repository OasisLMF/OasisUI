...
defineID <- function(input, output, session,
  ...
  # > modal content
  sub_modules$oasisui_analyses <- callModule(
    oasisuiTable,
    id = "oasisui_analyses",
  ...
  AnaList <- modalDialog(
    easyClose = TRUE,
    size = "l",
    oasisuiTableUI(ns("oasisui_analyses")),
    footer = tagList(
  ...
  observeEvent(sub_modules$oasisui_analyses$rows_selected(), ignoreNULL = FALSE, {
    if (is.null(sub_modules$oasisui_analyses$rows_selected())) {
  ...
  observeEvent(input$abuttonselectAna, {
    removeModal()
    withModalSpinner(
      .downloadOutput(idx = sub_modules$oasisui_analyses$rows_selected()),
      "Loading...",
      size = "s"
    )
  })
  ...
  .preselectAna <- function(preselAna) {
    if (!is.null(result$tbl_analysesData) && nrow(result$tbl_analysesData) > 0 ) {
      ...
      if (length(idx) > 0 &&
          ...
          !isTRUE(all.equal(sub_modules$oasisui_analyses$rows_selected(), idx))) {
