...
Credentials will be provided by the administrator. Note that there are some differences between administrator and non-admin modes. Under the first more information will be displayed in some of the tables, however functionalities of the UI do not vary depending on credentials. 
Tables summarize information regarding portfolio and analyses, where their statuses are represented by icons:
...
```
- Failed: something went wrong during the processing (e.g. wrong model, uploaded files, ...)
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
...
```
  - Collapse: window is expanded. Once clicked, the icon will change to the collapse button 
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
...
```
  - Pop-up list button: to select a previously run analysis 
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/icons/stripy_btn.png")
```

  - View: contents of files can be viewed by selecting the row and clicking on the button. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
```
Starting from the top, the user icon prompts the log out
...
```
The OASIS logo will re-direct to the OASIS website. 
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/logo/oasis_logo.png")
```
The panel right below lists the sections available in the UI
```{r, fig.align='center', out.width = "150px", out.height = "150px", echo = FALSE}
...
Several functionalities are integrated within:
  ...
  - The Home button on the top left will re-direct the user to this view at any point. 
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/landing_page/home_btn.png")
```

  - The collapse button will minimize the dropdown window to give more space to the other panels:
```{r, fig.align='center', out.width = "30px", out.height = "30px", echo = FALSE}
...
- Analysis table
This dropdown can accessed at any point during the workflow to re-direct the user to any section of the UI.
...
### Choose Portfolio
The first step involves choosing a portfolio. In case no portfolio was previously created, the table will show as empty. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/step_1/step_1.png")
```
To pick a previously created portfolio with no need of further changes, the corresponding row in the Portfolios table has to be selected, then the second step can be accessed by clicking on **Proceed to Choose Analysis** button.
...
```
After the portfolio has been labeled, it is possible to upload documents, the only required one is the Location file. In case a portfolio was previously created and new files need to be uploaded, this panel can be accessed by clicking on **Upload Source Files**.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
```
To pick a previously created analysis, the corresponding row in the Analyses table has to be selected, then the third step can be accessed by clicking on **Proceed to Configure Output & Run** button.

In order to create a portfolio, a name and a model are mandatory. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/step_2/create_analysis.png")
```

This is the only step where a model can be selected and this action cannot be reverted. To attach a different model a new analysis has to be created. Model information will depend on what the administrator has set up, more information such as model settings and hazard maps (if available) can be found by clicking on **Show Model Details**.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
There could be several reasons for the latter case, for example the chosen model or file was wrong or there could be some temporary issues on the back-end API. Detail of the failed analysis can be viewed in the log file. Once the status is "ready", the Validation Summary and Map as well as the inputs can be viewed.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
```
by clicking on **Output Configuration**. In case that an analysis was already run, the label of the button will switch to **Rerun** and the panel will display the previous configuration. The Output configuration includes choices of perspectives, summary levels, reports and model parameters. 

On the left the Model Parameters shows the available selection for the chosen model  
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
On the right the Output Parameters panel shows all options for tags, perspectives, summary levels and reports. If no previous configuration was selected for the analysis, the default tag is set to **Summary**, which includes *All Risks* for the Summary level and *AAL*, *LEC Full Uncertainty OEP* and *LEC Full Uncertainty AEP* for the Reports. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
```
It is possible to add rows for more combinations. In case that multiple entries are selected in one field, they will be aggregated. The **Clear** button will erase the entire selection. At the bottom, the table will show all combinations. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
```
It is also possible to duplicate the selection used in another analysis by clicking on the pop-up list button on the right. 
...
# View results 
The Dashboard section compiles results from the run and allows the user to create additional plots and view input and output files. Functionalities in this page can only be accessed if an analysis was successful. This view can be reached from the Home page either by selecting the analysis from the Analyses Table and clicking on **Dashboard of Analyses Outputs** or by clicking on **Dashboard** → **Single Analysis Dashboard** in the drop-down. 
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/landing_page/go_to_dashboard.png")
```
The analysis can be chosen or changed at any time by clicking on the pop-up list button on the top left. It comprises of 3 tabs Summary, Plots, and Files. 

### Summary
The first tab is the Summary, which shows the Inputs, Parameters and Outputs tables, as well as plots for any selection of OEP, AEP and AAL for both Analytical and Sample types. The OEP and AEP curves are plotted separately for all of the chosen perspectives.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/dashboard/dashboard.png")
```

### Plots
Additional plots can be created in the Plots tab. However, plots will not be stored, hence as soon the page is changed the plots will disappear.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/dashboard/dashboard_custom_plot.png")
```

It is mandatory to choose either *loss per return period* or *AAL bar plot*, which will respectively produce a line and a bar plot. Additionally, Perspectives, Reports and Summary Levels previously selected in the step 3 are available. Multiple selections of either Perspectives or Reports is possible and, if available, both Sample and Analytical types. 

```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
knitr::include_graphics("~/RStudioProjects/OasisLMFUI/BFE_RShiny/oasisui/documentation/dashboard/dashboard_plot_type.png")
```
In the case of AAL, uncertainty bars can also be added and in case that multiple Reports, Types or Perspectives are selected, an equivalent number of plots will be displayed. The legend shows all of the summary IDs retrieved from the Summary Levels. A different title other than automatically generated one can be typed and a maximum of five panels can be added.
```{r, fig.align='center', out.width = "200px", out.height = "200px", echo = FALSE}
...
The content of each file can be viewed in a separate pop-up and be exported as zip.
